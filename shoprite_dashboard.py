...
gg_initiatives.remove('Other initiatives (text)') # Remove text field for radar

gg_data = []
for country_code in countries_to_display:
    country_name = survey_data[country_code]['store_info']['country']
    for initiative in gg_initiatives:
        rank = survey_data[country_code]['going_green'][initiative]
        # Invert rank for radar: 1 (most evidence) -> 3, 3 (no evidence) -> 1
        # This way, a larger area on radar means more evidence.
        # Or, keep as is: smaller number is better. Let's keep as is (lower = better)
        gg_data.append({'Country': country_name, 'Initiative': initiative, 'Rank': rank})

if gg_data:
    gg_df = pd.DataFrame(gg_data)
    fig = go.Figure()
    colors = {'South Africa': 'rgba(0,100,80,0.7)', 'Zambia': 'rgba(255,140,0,0.7)'} # Greens, Oranges
    ...
    for country in gg_df['Country'].unique():
        ...
        fig.add_trace(go.Scatterpolar(
            ...
            fill='toself',
            name=country,
            line_color=colors.get(country, 'grey')
        ))
